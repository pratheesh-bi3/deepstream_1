...
def decodebin_pad_added(decodebin, pad, streammux):
    """
    Dynamic linking function for decodebin to connect to streammux.
    """
...
def main(args):
    ...
    decodebin = Gst.ElementFactory.make("decodebin", "decoder")  # <-- Now correctly defined
    ...
    if not all([source, decodebin, streammux, pgie, nvvidconv, nvosd, encoder, parser, muxer, sink]):
        sys.stderr.write("Error: Failed to create one or more elements\n")
        return
    ...
    sink.set_property("location", "output_3.mp4")
